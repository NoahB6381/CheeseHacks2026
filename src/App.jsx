...
const DEMO_ACCOUNTS = [
  { id: 1, name: 'Bucky Badger', username: '@bucky99',  email: 'bucky@wisc.edu',  bio: 'Pass-first point guard.'           },
  { id: 2, name: 'Sarah Miller', username: '@sarahm',   email: 'sarah@wisc.edu',  bio: 'Sharp shooter, 3-point specialist.' },
  { id: 3, name: 'Mike Torres',  username: '@miket',    email: 'mike@wisc.edu',   bio: 'Defensive stopper. Hustles every play.' },
];

// Hourly options 5 AM → midnight for the open/close pickers
const HOUR_OPTIONS = (() => {
  const opts = [];
  for (let h = 5; h <= 24; h++) {
...
// ─── Sub-component: Hour Pill Picker ─────────────────────────────────────────
          ...
          return (
            <button
              ...
              className={`py-1.5 text-xs font-bold rounded-lg border transition-all
                ${selected
                  ? 'bg-red-600 border-red-600 text-white shadow-md scale-[1.03]'
                  : disabled
...
export default function App() {
  ...
  const currentAccount = DEMO_ACCOUNTS.find(a => a.id === currentAccountId);
  ...
  const timeSlots = selectedLocation
    ? generateTimeSlots(selectedLocation.openTime, selectedLocation.closeTime)
    : ['Right Now'];
  ...
  const handleAddLocation = (e) => {
    ...
    const loc = { id: Date.now(), name: locationForm.name.trim(), openTime: locationForm.openTime, closeTime: locationForm.closeTime };
  ...
  const renderGameCard = (game) => {
    ...
    return (
      <div
        key={game.id}
        className={`bg-white rounded-xl border shadow-sm p-4 transition-all hover:shadow-md relative overflow-hidden
          ${isJoined ? 'border-green-200 bg-green-50/20' : 'border-gray-200'}`}
      >
        <div
  ...
  return (
    <div className="min-h-screen bg-gray-50 font-sans text-gray-900 pb-20" onClick={closeDropdowns}>
      ...
      <header className="bg-red-700 text-white sticky top-0 z-20 shadow-md">
        <div className="max-w-3xl mx-auto px-4 py-4 flex justify-between items-center">
          ...
          <div className="flex items-center space-x-4">
            <button onClick={e => { e.stopPropagation(); setShowFriendsModal(true); }} className="text-red-200 hover:text-white transition"><UserPlus className="w-5 h-5" /></button>
      ...
      {/* Demo switcher */}
      <div className="bg-gray-900 text-white text-xs py-2 px-4 flex items-center justify-center gap-3 flex-wrap">
      ...
      <main className="max-w-3xl mx-auto">
        ...
        <div className="bg-white p-4 shadow-sm border-b border-gray-100 relative z-10">
          <div className="grid grid-cols-2 gap-3">
            ...
            <div className="relative">
              ...
              <button
                onClick={e => { e.stopPropagation(); setLocationOpen(o => !o); setSportOpen(false); }}
              ...
              {locationOpen && (
                <div className="absolute top-full left-0 w-72 mt-1 bg-white border border-gray-100 rounded-xl shadow-xl overflow-hidden z-30"
                  ...
                  {locations.length === 0 && (
                    <div className="px-4 py-3 text-xs text-gray-400 text-center">No locations added yet.</div>
                  )}
                  {locations.map(loc => (
                    <div key={loc.id}
                      ...
                      <div className="flex-1 cursor-pointer min-w-0"
                        ...
                        <p className={`text-sm font-semibold truncate ${selectedLocationId === loc.id ? 'text-red-700' : 'text-gray-800'}`}>{loc.name}</p>
                      ...
                      <button onClick={() => handleRemoveLocation(loc.id)}
                        className="ml-3 text-gray-200 hover:text-red-500 transition opacity-0 group-hover:opacity-100 flex-shrink-0" title="Remove location">
                        <Trash2 className="w-4 h-4" />
                      </button>
                    </div>
                  ))}
            ...
            <div className="relative">
              ...
              {sportOpen && (
                <div className="absolute top-full right-0 w-full mt-1 bg-white border border-gray-100 rounded-xl shadow-xl overflow-hidden z-30"
                  onClick={e => e.stopPropagation()}>
                  <div onClick={() => { setSelectedSport('Basketball'); setSportOpen(false); }} className="p-3 cursor-pointer hover:bg-orange-50 text-sm font-semibold border-b border-gray-50">Basketball</div>
        ...
        <div className="flex px-4 pt-5 pb-2">
          {[{ key: 'find', label: 'Find a Squad', Icon: Search }, { key: 'create', label: 'Create a Squad', Icon: Plus }].map(({ key, label, Icon }) => (
                ...
                <div className="text-center py-14">
                  {!selectedLocationId ? (
                    ...
                    <>
                      ...
                      <p className="text-gray-400 font-medium text-sm">No squads at this location yet.</p>
          ...
          <div className="bg-white rounded-xl shadow-xl w-full max-w-sm overflow-hidden max-h-[90vh] flex flex-col">
            ...
            <form onSubmit={handleAddLocation} className="p-5 space-y-5 overflow-y-auto flex-1">
              ...
              <div>
                ...
                <input type="text" placeholder="e.g. Nicholas Recreation Center" required
              ...
              {/* Closes At pill picker */}
              <HourPicker
                label="Closes At"
                value={locationForm.closeTime}
                minVal={locationForm.openTime}
      ...
      {showProfileModal && (
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-xl shadow-xl w-full max-w-sm overflow-hidden">
            <div className="bg-red-700 p-4 text-white flex justify-between items-center">
              <h3 className="font-bold flex items-center gap-2"><User className="w-5 h-5" /> Player Profile</h3>
              <button onClick={() => setShowProfileModal(false)}><X className="w-5 h-5 text-red-200 hover:text-white" /></button>
            </div>
            <div className="p-5 space-y-4">
              <div className="flex justify-center">
                <div className="w-16 h-16 bg-red-100 rounded-full flex items-center justify-center text-red-600 text-2xl font-black">
                  {currentAccount.name.charAt(0)}
                </div>
              </div>
              <div className="text-center">
                <p className="font-bold text-gray-900">{currentAccount.name}</p>
              ...
              <div className="border-t pt-4">
                ...
                <div className="space-y-2">
                  {DEMO_ACCOUNTS.map(acc => (
                    <button key={acc.id}
                      onClick={() => { setCurrentAccountId(acc.id); setShowProfileModal(false); toast(`Switched to ${acc.name}`); }}
                      ...
                      <div className="flex items-center gap-3">
                        <div className={`w-8 h-8 rounded-full flex items-center justify-center font-bold text-sm
                          ${currentAccountId === acc.id ? 'bg-red-600 text-white' : 'bg-gray-100 text-gray-600'}`}>
                          {acc.name.charAt(0)}
                        </div>
